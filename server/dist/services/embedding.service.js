...
class EmbeddingService {
    ...
    static async embedWithGemini(text) {
        const apiKey = process.env.GEMINI_API_KEY;
        if (!apiKey) {
            throw new Error("GEMINI_API_KEY is required for embedding generation. Set it in your environment variables.");
        }
        ...
        const embedding = raw?.values ?? (Array.isArray(raw) ? raw : []);
        if (!embedding || embedding.length === 0) {
            throw new Error("Gemini embedding API returned an empty embedding vector");
        }
        return embedding;
    }
    static async embedWithOpenAICompatible(text) {
        const apiKey = process.env.EMBEDDING_API_KEY || process.env.OPENAI_API_KEY;
        const baseURL = process.env.EMBEDDING_BASE_URL || "https://api.openai.com/v1";
        const model = process.env.EMBEDDING_MODEL || "text-embedding-3-small";
        if (!apiKey) {
            throw new Error("EMBEDDING_API_KEY or OPENAI_API_KEY is required when using an OpenAI-compatible embedding provider.");
        }
        ...
        if (!response.ok) {
            const errorBody = await response.text();
            throw new Error(`OpenAI-compatible embedding API error (${response.status}): ${errorBody}`);
        }
        const data = await response.json();
