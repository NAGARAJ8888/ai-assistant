...
async function main() {
    ...
    try {
        ...
        const userMsg = await message_service_1.MessageService.saveUserMessage(conversation.id, question);
        ...
        const answer = "Based on the uploaded documents, total sales for Q3 were $2.4 million, representing a 15% increase year-over-year.";
        const sources = [
            {
        ...
        const assistantMsg = await message_service_1.MessageService.saveAssistantMessage(conversation.id, answer, sources);
        ...
        const retrieved = await conversation_service_1.ConversationService.getById(conversation.id, userId);
        if (!retrieved) {
        ...
        if (retrieved.messages.length !== 2) {
            throw new Error(`Expected 2 messages, got ${retrieved.messages.length}`);
        }
        ...
        const messages = await message_service_1.MessageService.getMessageHistory(conversation.id, userId);
        if (!messages) {
        ...
        const userConversations = await conversation_service_1.ConversationService.getByUser(userId);
        ...
        const deleted = await conversation_service_1.ConversationService.delete(conversation.id, userId);
        if (!deleted) {
        ...
        const afterDelete = await conversation_service_1.ConversationService.getById(conversation.id, userId);
        if (afterDelete !== null) {
        ...
        const afterDeleteMessages = await message_service_1.MessageService.getMessageHistory(conversation.id, userId);
